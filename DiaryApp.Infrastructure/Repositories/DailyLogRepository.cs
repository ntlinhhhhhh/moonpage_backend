...
public class DailyLogRepository : IDailyLogRepository
{
    ...
    private DailyLog MapSnapshotToLog(DocumentSnapshot snapshot)
    {
        var log = new DailyLog
        {
            ...
            Calories = snapshot.ContainsField("Calories") ? snapshot.GetValue<int>("Calories") : 0,
            Distance = snapshot.ContainsField("Distance") ? snapshot.GetValue<double>("Distance") : 0,
            MusicTitle = snapshot.ContainsField("MusicTitle") ? snapshot.GetValue<string>("MusicTitle") : null,
            ArtistName = snapshot.ContainsField("ArtistName") ? snapshot.GetValue<string>("ArtistName") : null,
            AlbumArtUrl = snapshot.ContainsField("AlbumArtUrl") ? snapshot.GetValue<string>("AlbumArtUrl") : null,
            ...
            WakeupTime = snapshot.ContainsField("WakeupTime") ? snapshot.GetValue<string>("WakeupTime") : null,
